...
rfe_select_features <- function(df, verbose = TRUE) {
  library(caret)
  library(dplyr)
  ...
  filtered_df <- df %>%
    select(results$optVariables) %>%
    mutate(death_from_cancer = df$death_from_cancer)
...
train_logistic_clinical_model_survival <- function(df) {
  library(caret)
  df <-
    subset(df, select = -overall_survival_months) # same as death_from_cancer
  df <- subset(df, select = -cohort) # not useful
  df <-
    subset(df, select = -integrative_cluster) # gene data (not in this dataset)
  
  # split data
  trainIndex <-
    createDataPartition(df$death_from_cancer,
                        p = .8,
                        list = FALSE,
                        times = 1)
  ...
  # make model
  model <-
    glm(death_from_cancer ~ ., data = trainData, family = 'binomial')
  model_stepped <- step(model, trace = FALSE, direction = "both")
  print(summary(model_stepped))
  print(model_stepped)
  # test model
  probabilities <-
    predict(model_stepped, newdata = testData, type = 'response')
  predicted_classes <- ifelse(probabilities > 0.5, "yes", "no")
  predicted_classes <-
    factor(predicted_classes, levels = c("no", "yes"))
  confMatrix <-
    confusionMatrix(predicted_classes, testData$death_from_cancer)
  print(confMatrix)
  
  save_model("clinical_logistic_survival.rds",
             confMatrix = confMatrix,
             model = model_stepped)
}
...
get_logistic_clinical_model_survival <- function() {
  loaded_model <-
    readRDS(file = "models/clinical_logistic_survival.rds")
  return (loaded_model)
}

## Naive Bayes Models
train_clinical_nb_model_survival <- function(df, featureSelection = FALSE) {
  ...
  model <-
    naiveBayes(death_from_cancer ~ ., data = trainData, laplace = 3)
  print(model)
  predictions <- predict(model, newdata = testData)
  confMatrix <-
    confusionMatrix(predictions, testData$death_from_cancer)
  print(confMatrix)
  
  save_model("clinical_nb_survival.rds",
             confMatrix = confMatrix,
             model = model)
}
...
train_clinical_dectree_model_survival <- function(df, cp = 0.001) {
  # Feature Selection not needed as tree already does it
  library(rpart)
  library(caret)
  df <-
    subset(df, select = -overall_survival_months) # same as death_from_cancer
  ...
  df <-
    subset(df, select = -cellularity) # removed because of bug I cant explain
  df <-
    subset(df, select = -neoplasm_histologic_grade) # removed because of bug I cant explain
  ...
  save_model("clinical_dectree_survival.rds",
             confMatrix = confMatrix,
             model = model)
}

get_clinical_dectree_model_survival <- function() {
  # Feature Selection not needed as rftree already does it
  library(rpart)
  loaded_model <-
    readRDS(file = "models/clinical_dectree_survival.rds")
  return (loaded_model)
}
...
train_clinical_rftree_model_survival <- function(df) {
  ...
  save_model("clinical_rftree_survival.rds",
             confMatrix = confMatrix,
             model = model)
}
...
get_clinical_rftree_model_survival <- function() {
  library(randomForest)
  loaded_model <-
    readRDS(file = "models/clinical_rftree_survival.rds")
  return (loaded_model)
}
...
train_gene_rf_model <- function(df) {
  library(randomForest)
  randomForest_model <-
    randomForest(death_from_cancer ~ ., data = genomic_data)
  saveRDS(randomForest_model,
          "models/gene_rf_death_from_cancer.rds")
}
