...
## Oversampling
if(balance_data) {
  n_samples <- sum(raw_data$death_from_cancer == "no") * 2
  over <- ovun.sample(death_from_cancer ~ ., data = raw_data, method = "over", N = n_samples)
  raw_data <- over$data 
}

raw_clinical_data <- raw_data[,1:24]
raw_gene_data <- raw_data[,24:ncol(raw_data)]
